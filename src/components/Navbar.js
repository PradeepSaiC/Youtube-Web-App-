...
const Navbar = () => {
  ...
  const suggestionsCache = useSelector( state => state.search)
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if(searchQuery in suggestionsCache){
           const list = suggestionsCache[searchQuery]
           setSuggestions(list);
      }else{
        const resultdispatcher = async()=> {
          const result = await getSuggestions()
                   dispatch(addSearchSuggestions({[searchQuery]:result}))
        }
        resultdispatcher();
      }
    }, 200);
  ...
  return (
    <div className="grid z-40 grid-flow-col px-5 py-3 h-22 w-full fixed bg-white shadow shadow-md">
...
<Link to={"/"}>
        ...
        <svg
          className="px-3py-1 my-auto w-28 cursor-pointer"
        ...
        <div className="relative">
          ...
          >
            ...
            onBlur={
              ...
              value={searchQuery}
              className="w-96 border border-gray-400 px-4 py-1 rounded-l-full focus:outline-none"
              type="text"
              placeholder="Search"
            />
            <button
              type="submit"
              className="bg-gray-300 border border-gray-400 px-2 py-[7px] pb-[9px] rounded-r-full hover:bg-gray-200"
            >
              <svg
                aria-hidden="true"
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute bg-white w-full mt-1 p-2 rounded-md border border-gray-400">
                <ul>
                  {suggestions.map((s) => (
                    <li
                      onClick={() => {
                      ...
                      key={s}
                      className="hover:bg-slate-100 cursor-pointer px-2 py-1 rounded-md flex items-center"
                    >
                      <span className="m-2">
                        <svg
                          aria-hidden="true"
